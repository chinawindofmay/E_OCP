    ...
    for row in rows:
        try:
            ...
            arcpy.Visibility_3d(dem_raster, solopoint_shp, save_outvisibility_raster, algOutput, "FREQUENCY", "ZERO",
                                ...
                                "", "")
            # out_Int_raster=Int(save_outvisibility_raster)
            print("可见性分析结束")
            # print("开始选择value值不为0的面")
            MakeRas_visibil1 = "MakeRas_visibil1"
            arcpy.MakeRasterLayer_management(save_outvisibility_raster, MakeRas_visibil1, "\"VALUE\" <> 0",
                                             save_outvisibility_raster, "1")
...
def step3_apppend_visi_shp_into_one_file(appeend_stand_shp):
    print("追加开始！！！！！！！！！！！！！！！！")
...
def step5_create_price_matrix_with_distance_decay_from_visi_shp_file_and_centre_points(zhixin_shp, appeend_stand_shp,
    ...
    # 初始化矩阵
    price_matrix = np.zeros([TOWER_COUNT, FID_max + 1], dtype=float)
    # intersect_shp的生成的步骤
    # 首先对候选点赋值塔高度，得到monitor_with_extra_height_shp点图层，会用到DEM，得到的是每个塔点位基座的海拔高度，增加KEY_ID字段；
    # 开始遍历monitor_with_extra_height_shp中的每一个点，
    #   进行可视域分析，输入数据包括：monitor_with_extra_height_shp中的每一个点、DEM、布设高度参数，输出数据是：栅格
    ...
    for row in zhixin_rows:
        ...
        price_matrix[int(TOWER_KEY_ID)][FID_zhixin] = RIDUS/distance    #以此作为反距离权重。
...
def create_spatial_price_matrix(
    ...
    step2_generate_every_point_visibility_shp(root_file_path, monitor_with_extra_height_shp, farm_land_shp, dem_raster,
                                              TOWER_HEIGHT, RIDUS)
    # 将所有可见区域追加到一个文件中
    step3_apppend_visi_shp_into_one_file(appeend_stand_shp)
    # #生成fishnet和中心点
    ...
    step5_create_price_matrix_with_distance_decay_from_visi_shp_file_and_centre_points(zhixin_shp, appeend_stand_shp,
                                                                   price_matrix_save_file,
                                                                   RIDUS,
                                                                   monitor_with_extra_height_shp)
...
主函数入口
"""
def create_cost_matrix(tower_points_shp, dem_raster, xzq, GD_land_shp, out_path, TOWER_HEIGHT, RIDUS, FISHNET_CELL_SIZE, capacity,
    ...
    create_spatial_price_matrix(root_file_path,
                                ...
                                FISHNET_CELL_SIZE,
                                zhixin_shp,
                                price_matrix_save_file,soft_price_matrix_save_file)
...
def debugger_test():
    ...
    create_cost_matrix(tower_points_shp, dem_raster, xzq, GD_land_shp, out_path, TOWER_HEIGHT, RIDUS, FISHNET_CELL_SIZE,
...
if __name__ == "__main__":
    ...
    #
    # create_cost_matrix(tower_points_shp, dem_raster, xzq, GD_land_shp, out_path, TOWER_HEIGHT, RIDUS, FISHNET_CELL_SIZE,
    #                    int(capacity), appeend_stand_shp, zhixin_shp, price_matrix_save_file,soft_price_matrix_save_file)
    # print("success")
    # # create()
